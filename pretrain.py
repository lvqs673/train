...
import argparse
import os
os.environ['CUDA_VISIBLE_DEVICES'] = '7,6,5'
...
def train_epoch(epoch, model, dataloader, creterion, optimizer):
    ...
    for batch_idx, (input_ids, target_ids, mask) in enumerate(dataloader, 1):
        ...
        if master and (batch_idx+1) % _print_interval == 0:
            print(f'\t{batch_idx}th batch - {batch_idx}/{n_batch} ({(batch_idx)/n_batch*100:.1f}%)')
            print(f'\t\tcurrent batch loss: {cur_batch_loss}, cost {time.time()-stt1} seconds.')
            cur_batch_loss = 0
            stt1=time.time()
    if local_rank==0:
        print(f'Epoch {epoch} cost {time.time()-stt0} seconds.\n')
...
if __name__ == "__main__":
    ...
    creterion = nn.CrossEntropyLoss(reduction='none')
    ...
    for epoch in range(1, n_epoch+1):
        train_epoch(epoch, model=ddp_model, dataloader=trainloader,
                    creterion=creterion, optimizer=optimizer)
        if master:
    ...
    if master:
        ...
        print(f'Totally cost {time.time()-stt} seconds.\n')
